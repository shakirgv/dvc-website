          ...
          {isLoading ? (
            ...
            programs.map((program, idx) => (
              ...
              >
                ...
                {program.image_url ? (
                  <div className="w-full h-48 rounded-2xl mb-6 overflow-hidden border border-border">
                    <img src={program.image_url} alt={program[`title_${locale}`]} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  </div>
                ) : (
                ...
                <h2 className="text-2xl font-bold mb-4 line-clamp-2">{program[`title_${locale}`]}</h2>
                <p className="text-muted-foreground mb-8 flex-1 line-clamp-4">
                  {program[`description_${locale}`]}
                </p>
                
                <Link 
                  href={program.link || `/${locale}/programs`}
